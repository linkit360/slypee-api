...
<div class="search-form">
    <?php $form = ActiveForm::begin([
        'layout' => 'horizontal',
        'method' => 'GET',
        'action' => Url::to(['log/', 'type' => 'all', 'user_id' => $user->id]),
        'options' => [
            'id' => 'search-form',
        ],
        'fieldConfig' => [
    ...
    <div class="row">
        ...
        <div class="cell cell_half">
            <?= $form->field($search, 'object_type')->dropDownList($search->types, [
    ...
    <div class="row flex flex_end">
        <?= Html::resetButton('Reset filter', ['class' => 'btn orange', 'style' => 'margin-right: 20px', 'onclick'=>"window.location = '".Url::to(['log/', 'type' => 'all', 'user_id' => $user->id])."'"]) ?>
    ...
    <table class="striped responsive-table">
        ...
        <?php foreach ($logs as $log): ?>
            <tr>
                ...
                <td><?= $log->user->username ?> <?= $log->id ?></td>
                <td>
                    <?=$log->object_type?> / <?=$log->object_id?> / <?=$log->objectName?>
                </td>
